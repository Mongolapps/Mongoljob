...
from database import (
    ...
    get_service, get_services_by_business, get_user, init_db, mark_match_paid, reject_job,
    save_user, set_booking_status, set_employer_decision, set_payment_requested, stats,
)
...
PAYMENT_INSTRUCTIONS = os.getenv("PAYMENT_INSTRUCTIONS", "Дансны мэдээллээ .env файлд оруулна уу.").strip()
if not BOT_TOKEN:
...
def job_text(j):
    ...
    return (f"{badge}🏢 <b>{escape(j['company'])}</b>\n\n"
            f"{type_label}\n💼 <b>{escape(j['title'])}</b>\n📂 {escape(j['category'])}\n"
            f"💰 {escape(j['salary'])}\n🕒 {escape(j['schedule'])}\n📍 {escape(j['location'])}\n\n"
...
async def p_salary(update, context):
    ...
    await update.message.reply_text("✅ Анкет хадгалагдлаа.", reply_markup=MAIN_MENU)
    return ConversationHandler.END
...
async def j_desc(update, context):
    ...
    if ADMIN_ID:
        await context.bot.send_message(ADMIN_ID, "🆕 <b>Шинэ ажлын зар</b>\n\n" + job_text(j), parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Энгийн", callback_data=f"job_free:{jid}"), InlineKeyboardButton("⭐ Premium", callback_data=f"job_premium:{jid}")], [InlineKeyboardButton("👑 VIP", callback_data=f"job_vip:{jid}"), InlineKeyboardButton("❌ Татгалзах", callback_data=f"job_no:{jid}")]]))
    return ConversationHandler.END
...
async def job_admin(update, context):
    q = update.callback_query
    if q.from_user.id != ADMIN_ID: await q.answer("Админ эрхгүй.", show_alert=True); return
    action, raw = q.data.split(":"); jid = int(raw); j = get_job(jid)
    if action == "job_free": ok = approve_job(jid, "free")
    elif action == "job_premium": ok = approve_job(jid, "premium", 7)
    elif action == "job_vip": ok = approve_job(jid, "vip", 30)
    else: ok = reject_job(jid)
    await q.answer("Шийдвэр хадгалагдлаа." if ok else "Өмнө шийдвэрлэсэн.", show_alert=True); await q.edit_message_reply_markup(None)
    if ok: await context.bot.send_message(j["employer_id"], "✅ Ажлын зар батлагдлаа." if action != "job_no" else "❌ Ажлын зар татгалзагдлаа.")
...
async def employer_decision(update, context):
    q = update.callback_query; action, raw = q.data.split(":"); mid = int(raw); m = get_match(mid)
    if not m or q.from_user.id != m["employer_id"]: await q.answer("Эрхгүй.", show_alert=True); return
    accepted = action == "emp_yes"; ok = set_employer_decision(mid, accepted)
    await q.answer("Шийдвэр хадгалагдлаа." if ok else "Өмнө шийдвэрлэсэн.", show_alert=True); await q.edit_message_reply_markup(None)
    if not ok: return
    if not accepted: await context.bot.send_message(m["applicant_id"], "ℹ️ Ажил олгогч Match хүсэлтийг үргэлжлүүлээгүй."); return
    if ADMIN_ID:
        await context.bot.send_message(ADMIN_ID, f"💳 Match #{mid}: хоёр тал зөвшөөрсөн.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{MATCH_PRICE:,}₮ нэхэмжлэх", callback_data=f"invoice:{mid}")]]))
async def invoice_callback(update, context):
    q = update.callback_query
    if q.from_user.id != ADMIN_ID: await q.answer("Админ эрхгүй.", show_alert=True); return
    mid = int(q.data.split(":")[1]); m = get_match(mid); ok = set_payment_requested(mid, MATCH_PRICE)
    if not ok: await q.answer("Нэхэмжлэл илгээгдсэн байна.", show_alert=True); return
    await context.bot.send_message(m["employer_id"], f"💳 <b>Холболтын төлбөр</b>\n\nMatch #{mid}\nДүн: <b>{MATCH_PRICE:,}₮</b>\n\n{escape(PAYMENT_INSTRUCTIONS)}", parse_mode=ParseMode.HTML)
    await q.edit_message_reply_markup(InlineKeyboardMarkup([[InlineKeyboardButton("✅ Төлбөр батлах", callback_data=f"paid:{mid}")]])); await q.answer("Нэхэмжлэл илгээлээ.")
async def paid_callback(update, context):
    q = update.callback_query
    if q.from_user.id != ADMIN_ID: await q.answer("Админ эрхгүй.", show_alert=True); return
    mid = int(q.data.split(":")[1]); m = get_match(mid)
    if not mark_match_paid(mid): await q.answer("Өмнө батлагдсан.", show_alert=True); return
    au = f"@{m['applicant_username']}" if m["applicant_username"] else "байхгүй"; eu = f"@{m['employer_username']}" if m["employer_username"] else "байхгүй"
    await context.bot.send_message(m["employer_id"], f"🎉 Холбоо нээгдлээ\n👤 {m['full_name']}\n☎️ {m['phone']}\nTelegram: {au}")
    await context.bot.send_message(m["applicant_id"], f"🎉 Match амжилттай\n🏢 {m['company']}\nTelegram: {eu}")
    await q.edit_message_reply_markup(None); await q.answer("Төлбөр баталгаажлаа.")
...
async def admin_stats(update, context):
    ...
    d = stats(); await update.message.reply_text("📊 ServiGo статистик\n\n" + "\n".join([
        ...
        f"⏰ Цагийн ажил: {d['part_time_jobs']}", f"🤝 Match: {d['matches']}", f"💰 Орлого: {d['revenue']:,}₮"
    ]))
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(match_callback, pattern=r"^match:\d+$")); app.add_handler(CallbackQueryHandler(interest_callback, pattern=r"^interest:\d+$")); app.add_handler(CallbackQueryHandler(business_admin, pattern=r"^biz_(yes|no):\d+$")); app.add_handler(CallbackQueryHandler(job_admin, pattern=r"^job_(free|premium|vip|no):\d+$")); app.add_handler(CallbackQueryHandler(booking_decision, pattern=r"^booking_(yes|no):\d+$")); app.add_handler(CallbackQueryHandler(employer_decision, pattern=r"^emp_(yes|no):\d+$")); app.add_handler(CallbackQueryHandler(invoice_callback, pattern=r"^invoice:\d+$")); app.add_handler(CallbackQueryHandler(paid_callback, pattern=r"^paid:\d+$"))
